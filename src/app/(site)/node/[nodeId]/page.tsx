...
const NodePage: React.FC = () => {
  ...
  const fetchPosts = async (): Promise<void> => {
    ...
    setLoading(true);
    try {
      const response = await Endpoints.getFeeds('node', nodeId, page);
  ...
  return (
    <div className="mt-4 flex flex-col items-center gap-3 w-full px-4">
      {posts.map((post) => (
        <PostComponent nodeId={nodeId} key={post._id} post={post} />
      ))}
...
const PostComponent: React.FC<PostComponentProps> = ({ post,nodeId }) => {
